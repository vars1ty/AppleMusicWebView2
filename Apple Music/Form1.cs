...
namespace Apple_Music
{
    public partial class Form1 : Form
    {
        ...
        private void AppleMusic_TitleChanged(object sender, object e)
        {
        ...
        {
            await webView.CoreWebView2.ExecuteScriptAsync("const elements = document.getElementsByClassName('web-navigation__native-upsell'); while (elements.length > 0) elements[0].remove();");
            await webView.CoreWebView2.ExecuteScriptAsync("while (elements.length > 0) elements[0].remove();");
        }

        private DiscordRichPresence rpc = new DiscordRichPresence();
        ...
        private async void AppleMusic_InitDiscordRPC(object sender, object e)
        {
            ...
            rpc.Initialize();
            // Get playing state
            await webView.CoreWebView2.ExecuteScriptAsync("MusicKit.getInstance().addEventListener( MusicKit.Events.playbackStateDidChange, (a) => {" +
                "window.chrome.webview.postMessage({state: a.state});" +
            "});");
            // Get music data. I'm so sorry for making you see this
            await webView.CoreWebView2.ExecuteScriptAsync("MusicKit.getInstance().addEventListener( MusicKit.Events.mediaItemStateDidChange, function() {" +
        ...
        private void AppleMusic_DiscordRPC(object sender, CoreWebView2WebMessageReceivedEventArgs args)
        {
            ...
            MusicKitResponse data = JsonSerializer.Deserialize<MusicKitResponse>(json, options);
